...
JSON_NAME="com.third_party.nlohmann_json.source@${PKG_VERSION}" # Имя с версией
...
#  git clone "${DEP_GIT_URL}" "${THIRD_PARTY_DEPS_SOURCES_DIR}/${DEP_NAME}.git" || { echo "Error: Failed to install ${DEP_NAME}."; exit 1; }
#  echo "Dependency ${DEP_NAME} install successfully."
#  echo "Updating and pinning ${DEP_NAME} to tag ${DEP_GIT_TAG}..."
#  cd "${THIRD_PARTY_DEPS_SOURCES_DIR}/${DEP_NAME}" || { echo "Error: Dependency directory for ${DEP_NAME} not found."; exit 1; }
#  git fetch origin
